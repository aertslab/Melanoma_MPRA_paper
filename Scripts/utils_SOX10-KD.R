process.enh.cDNA.plasmid <- function(env, 
  ...
  # and only keep enhancers if they have a min number of rBC linked, and store in plasmid.aggregated
  ...
  for (i in c("Human","Pig","Horse","Dog","Mouse","Zebra","Neg-Control")){
  env[[line]]$merged[grep(pattern = i, x = env[[line]]$merged$synthetic.region), "class"] <- i
  }
  # Rename Zebra class to Zebrafish
  env[[line]]$merged[env[[line]]$merged$class == "Zebra","class"] <- "Zebrafish"
  ...
  shuffled <- env[[line]]$merged.input.norm[env[[line]]$merged.input.norm$class == "Neg-Control",]
